...
function DrawPointTour () {

    for (var key in tourData.points) {
            // if (event[key].id == id) {
                console.log();
                    $('.tour_map_about_place_box_title').text(tourData.points[0].name);
                    $('.tour_map_about_place_box p').text(tourData.points[0].descriptionPoint);
                var marker = new google.maps.Marker({
                
                    position: {lat: Number(tourData.points[key].latitude), lng: Number(tourData.points[key].longtitude)},
                    map: map,
                    icon: {
                ...
                tourData.points[key].marker = marker;

                google.maps.event.addListener(marker,'click', (function(marker, name, descriptionPoint, infowindow, id){
    
                    return function() {
                    $('.tour_map_about_place_box_title').text(name);
                    $('.tour_map_about_place_box p').text(descriptionPoint);
                ...
                })(marker, tourData.points[key].name, tourData.points[key].descriptionEvent, infowindow, tourData.points[key].id));
    ...
    render () {
        return (
            <ul>
                ...
                <li><a href="http://s2t.loc/index.php?r=tour%2Findex"> Події </a> /</li>
