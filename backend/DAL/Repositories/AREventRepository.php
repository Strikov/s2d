...
class AREventRepository implements IEventRepository
{
    ...
    public function getCarousel($id_city)
    {
        ...
        if (!$events = Event::find()->select(['id', 'name', 'date', 'image', 'descriptionEvent'])->where(['city' => $id_city])->orderBy(['date' => SORT_ASC, 'begin' => SORT_ASC])->limit(10)->all()) {
            $events = Event::find()->select(['id', 'name', 'date', 'image', 'descriptionEvent'])->orderBy(['date' => SORT_ASC, 'begin' => SORT_ASC])->limit(10)->all();
        }
        if (!$events)
    ...
    public function getOneView($id)
    {
        $event = $this->get($id);
        $this->formatData($event, 1980, 1080);
        $event->category = $event->category->getName();
        return $event;
